...
> **Status: current, but the sink moved.** The `relay/` directory in this repo
> was deleted when Bento shrank to a plain-SSH tool, and the *transport* relay
> it contained is gone for good. What batches are POSTed to now is a separate
> service at `https://relay.bentoai.dev`, deployed from its own repository,
> whose main job is holding the provider keys that make voice work with nothing
> configured. Telemetry rides along on it. Paths below that point into `relay/`
> refer to that other repository.
...
| `ssh_direct_connected` | a plain SSH connection succeeded (iOS only) |
...
it already inherently serves** (ASR mint/socket/batch calls, per engine).
These involve no client-side collection and never store IPs or content. The
daemon- and pairing-related counters that used to sit beside them went away
with those features.
