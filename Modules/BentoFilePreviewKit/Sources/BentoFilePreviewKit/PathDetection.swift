...
///
/// Coordinate model: `read_text(SCREEN)` returns LOGICAL lines — ghostty joins
/// soft-wrapped rows back into one line — while taps land in VISUAL (wrapped)
/// row space, top-aligned with the SCROLLBAR action's `offset`/`total`.
/// `PathHitTester` bridges the two with the same `ceil(displayCells / cols)`
/// accumulation, then maps the in-row cell column to a character of the logical
...
public enum PathDetector {
    ...
    /// accounting `TerminalGeometry.displayCells` uses, so hit-testing and wrap
    /// math can never disagree).
    private static func charCells(_ ch: Character) -> Int {
        ch.unicodeScalars.reduce(0) { $0 + TerminalGeometry.scalarCells($1.value) }
    }
...
public struct PathHitTester {
    ...
    public init(screenText: String, cols: Int) {
        ...
        for piece in screenText.split(separator: "\n", omittingEmptySubsequences: false) {
            ...
            lines.append(s)
            starts.append(row)
            row += TerminalGeometry.visualRows(s, cols: cols)
        }
        self.lines = lines
        self.starts = starts
