...
/// that ran out of budget before the tree was exhausted.
public struct PaletteFileResult: Sendable {
    public let items: [PaletteItem]
    /// True when the fuzzy walk was cut off by its budget — rows are
    /// best-so-far, not exhaustive.
    public let partial: Bool
    public let dirsSearched: Int
}

/// Turns (query, browse-root, pane context) into file/dir rows — the palette's
/// live File section. Reuses the tap-preview infrastructure: `DirectoryListCache`
/// for per-directory listings (the lazy incremental index) and a budgeted
/// `TreeWalker` walk for fuzzy search — no pre-listed whole-tree index, so
/// depth is unlimited. Directories drill; files preview.
///
/// - Empty query: immediate children of `root` (dirs first) — a browser.
/// - Typed query: fuzzy walk over the subtree under `root` (budgeted; the
///   palette is an explicit search, so it gets a bigger budget than a tap).
...
public enum PaletteFileBrowser {
    public static func items(query: String, root: String,
                             context: PathPreviewContext, limit: Int = 40) async -> PaletteFileResult {
        let trimmed = query.trimmingCharacters(in: .whitespaces)
        ...
        if trimmed.hasPrefix("/") || trimmed.hasPrefix("~") {
            if let (resolved, stat) = try? await context.source.stat(path: trimmed, cwd: root) {
                return PaletteFileResult(
                    items: [fileItem(relPath: (resolved as NSString).lastPathComponent,
                                     absPath: resolved, isDir: stat.isDirectory)],
                    partial: false, dirsSearched: 0)
            }
            return PaletteFileResult(items: [], partial: false, dirsSearched: 0)
        }

        if trimmed.isEmpty {
            // Browse: immediate children only, directories first, alphabetical.
            guard let listing = try? await DirectoryListCache.shared.listing(
                source: context.source, path: root, request: .init()) else {
                paletteLog.log("file listing unavailable root=\(root, privacy: .public)")
                return PaletteFileResult(items: [], partial: false, dirsSearched: 0)
            }
            let children = listing.entries.sorted { a, b in
                if a.isDir != b.isDir { return a.isDir && !b.isDir }
                return a.name.localizedCaseInsensitiveCompare(b.name) == .orderedAscending
            }
            return PaletteFileResult(
                items: children.prefix(limit).map {
                    fileItem(relPath: $0.name, absPath: join(root, $0.name), isDir: $0.isDir)
                },
                partial: false, dirsSearched: 1)
        }

        // Fuzzy over the subtree — a lazy budgeted walk. Only entries the
        // fuzzy scorer could rank are collected (the walker's predicate), so
        // the collect cap never starves a real match.
        let outcome: SearchOutcome
        do {
            outcome = try await TreeWalker.walk(
                source: context.source, root: root,
                include: { relPath, _ in
                    let base = (relPath as NSString).lastPathComponent
                    return PaletteFuzzy.score(query: trimmed, target: base) != nil
                        || PaletteFuzzy.score(query: trimmed, target: relPath) != nil
                },
                request: SearchRequest(maxDirs: 512, maxChildrenPerDir: 200,
                                       timeBudget: 2.0, parallelism: 1))
        } catch {
            paletteLog.log("file search failed root=\(root, privacy: .public): \(String(describing: error), privacy: .public)")
            return PaletteFileResult(items: [], partial: false, dirsSearched: 0)
        }
        ...
        return PaletteFileResult(
            items: scored.prefix(limit).map {
                fileItem(relPath: $0.1.relPath, absPath: join(root, $0.1.relPath), isDir: $0.1.isDir)
            },
            partial: outcome.partial, dirsSearched: outcome.dirsVisited)
    }
