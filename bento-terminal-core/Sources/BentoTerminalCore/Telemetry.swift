...
/// `CLIENT_EVENTS` allowlist — keep it publishable, and keep the two lists in
/// sync. (The relay lives in its own repository now, so that mirror is no
/// longer a path in this tree.)
///
/// Never add anything that could carry terminal content, commands,
/// transcripts, paths, or hostnames.
///
/// An event whose feature has been deleted must be deleted too: this list is
/// rendered to users as a promise about what we measure, so a name that cannot
/// possibly fire devalues the whole list. `pairing_succeeded` was exactly that
/// after the pairing flow went away.
...
public final class TelemetryService: ObservableObject {
    ...
    private static let defaultRelayBaseURL = "https://relay.bentoai.dev"
